...
let read_byte p =
  peek_byte p |> and_skip p 1

let read_word ?(be=false) p =
  let cat b0 b1 = (b0 lsl 8) lor b1 in
  let b0 = read_byte p in
  let b1 = read_byte p in
  (if be then cat b0 b1 else cat b1 b0) |> and_skip p 2

let read_dword ?(be=false) p =
  let cat w0 w1 =
    Int32.(logor (shift_left (of_int w0) 16) (of_int w1)) in
  let w0 = read_word ~be p in
  let w1 = read_word ~be p in
  (if be then cat w0 w1 else cat w1 w0) |> and_skip p 4

let read_qword ?(be=false) p =
  let cat d0 d1 =
    Int64.(logor (shift_left (of_int32 d0) 32) (of_int32 d1)) in
  let d0 = read_dword ~be p in
  let d1 = read_dword ~be p in
  (if be then cat d0 d1 else cat d1 d0) |> and_skip p 8

let read_bytes p l =
  assert (Bytes.length p.data - p.offset >= l) ;
  Bytes.sub p.data p.offset l |> and_skip p l
