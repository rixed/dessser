...
and e0 =
   ...
   * name is used verbatim by the backend, no question asked, and must therefore
   * correspond to a valid object for that backend, identifying an object of the
   * given type. *)
  | ExtIdentifierUnmanaged of string
  (* We also want to be able to call functions generated by dessser itself
   * regardless of the backend so for this we need a specific abstraction: *)
  | ExtIdentifierManaged of {
      (* The name of the (external) type of the encoded/decoded/transcoded
       * value. *)
      type_name : string ;
      (* [meth] describes the operation that's requested. The backend will know
       * what identifier corresponds to this, with proper module nane/namespace. *)
      meth : type_method }
  | Now
  | RandomFloat
  | RandomU8
...
let rec string_of_e0 = function
  ...
  | ExtIdentifierUnmanaged s -> "ext-identifier "^ String.quote s
  | ExtIdentifierManaged { type_name ; meth } ->
      "ext-identifier "^ type_name ^" "^ string_of_type_method meth
...
(*
 * To be able to type ExtIdentifierManaged expressions the typer needs to know
 * the type of augmented pointers used by any specific encoders. But that would
 * introduce circular dependencies if it were merely getting that value from
 * the encoders themselves.
 * So instead each encoder has to register itself here.
 * Unregistered encoders are assumed to use naked ptrs.
 *)

let ptr_types_of_enc : (encoding_id, mn * mn) Hashtbl.t = Hashtbl.create 10

let register_ptr_type enc sptr dptr =
  assert (not (Hashtbl.mem ptr_types_of_enc enc)) ;
  Hashtbl.add ptr_types_of_enc enc (sptr, dptr)

let sptr_of_enc enc =
  try Hashtbl.find ptr_types_of_enc enc |> fst
  with Not_found -> ptr

let dptr_of_enc enc =
  try Hashtbl.find ptr_types_of_enc enc |> snd
  with Not_found -> ptr
