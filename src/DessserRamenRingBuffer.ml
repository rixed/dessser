...
(* We use a stack of "frames" of pointer to fieldmask + fieldbit index, where
 * the fieldmask points at a bitmask with one bit per field, set if the field
 * value is actually present (non default).
 * Every fieldmask starts with a byte giving ther length in words of this
...
(* Set the next bit in the fieldmask and return the new stack with increased
 * fieldbit position: *)
...
(* Skip a bit in the fieldmask, ie just increment the fieldbit (since the
 * fieldmask is initialized to zero), and return as above the stack with
 * that new fieldbit. *)
...
struct
  ...
  let rec of_type = function
    | T.TVec (dim, _) ->
        vec_bits dim
    | TArr _ ->
        (* Arrays fieldmask is dynamic and prefixed with a length *)
        invalid_arg "FieldMaskWidth.of_type for lists"
    | TTup mns ->
        tup_bits mns
    | TRec mns ->
        rec_bits mns
  ...
  (* Return the number of bytes required to encode the fieldmask *)
  let bytes_of_type typ =
    let bits = of_type typ in
    round_up_const_bits (bits + 8) (* with prefix length *)

  (* Return the number of words required to encode the fieldmask *)
  let words_of_type =
...
struct
  ...
  (* Zero a fieldmask which width is known at compile time, and advance the
   * pointer. *)
  let zero_fieldmask_const bits p =
    let sz = (bits + 7) / 8 in
  ...
  (* Zero the fieldmask known only at runtime (which size in words a given
   * in the first byte of the mask) and advance the pointer *)
  let zero_fieldmask_dyn bits p =
    let_ ~name:"sz_" (right_shift (add (u32_of_int 7) bits) (u8_of_int 3))
      (fun sz ->
        (* The fieldmask is prefixed with a byte for the length (in words): *)
  ...
  (* Enter a new compound type by zeroing a fieldmask of the given [max_width]
   * and setting up a new frame for it, all this after having set its own
   * fieldbit. *)
  let enter_frame to_expr zero_fieldmask max_width p_stk =
    E.with_sploded_pair "enter_frame" p_stk (fun p stk ->
      ...
      seq [ debug (string "ser: enter a new frame at ") ;
            ...
            make_pair
              (zero_fieldmask max_width p)
              (cons new_frame stk) ])

  (* Enter a new compound type by zeroing a fieldmask and setting up a new
   * frame for it, all this after having set its own fieldbit: *)
  let enter_frame_dyn = enter_frame identity zero_fieldmask_dyn
  let enter_frame_const = enter_frame u8_of_int zero_fieldmask_const
  ...
  let start _conf p =
    ...
    let new_frame = make_pair p (size 8 (* width of the length prefix *)) in
    make_pair (zero_fieldmask_const 1 p) (cons new_frame stk)
  ...
  let tup_rec_opn mns _ _ p_stk =
    ...
    let bits = FieldMaskWidth.tup_bits mns in
   ...
   * - a 16bits fieldmask, of which only bit 0 will ever be used
  ...
  let vec_opn dim _mn () _ _ p_stk =
    let bits = FieldMaskWidth.vec_bits dim in
    enter_frame_const bits p_stk
  ...
  let arr_opn _mn n () _ _ p_stk =
    ...
    let bits = FieldMaskWidth.lst_bits n in
    let p_stk =
  ...
  (* The fieldmask has been zeroed already: *)
  ...
  let ssize_of_arr _ _ id =
    let fieldmask_bits_dyn = cardinality id in
    (* Add the fieldmask length prefix: *)
    let fieldmask_sz_bits = add fieldmask_bits_dyn (u32_of_int 8) in
    (* Round up to ringbuf words: *)
    let fieldmask_bytes = round_up_dyn_bits fieldmask_sz_bits in
  ...
  let ssize_of_tup mn0 path  _ =
    ...
    let fieldmask_words =
      FieldMaskWidth.words_of_type (Path.type_of_path mn0 path).typ in
    size (fieldmask_words * word_size)
...
struct
   ...
   * index.
   * The fieldbit index of the new stack is set to 8 after the fieldmask
  ...
  (* Sums are encoded with a leading u16 for the fieldmask followed by
   * the label as a u16: *)
